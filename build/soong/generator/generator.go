...
func init() {
	android.RegisterModuleType("altermis_generator", GeneratorFactory)
...
type generatorProperties struct {
	...
	// prebuilts or scripts that do it need a module to build them.
	Tools []string
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = altermisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = altermisExpandVariables(ctx, dep_file)
	...
	exCmd := altermisExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "altermis.sbox.textproto"
	desc := "generate"
	name := "generator"
